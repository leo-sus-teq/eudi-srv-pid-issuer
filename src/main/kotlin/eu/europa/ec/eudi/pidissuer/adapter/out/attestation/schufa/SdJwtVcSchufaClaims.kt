...
object SdJwtVcSchufaClaims {
    ...
    val CreditScore: ClaimDefinition =
        ClaimDefinition(
            path = ClaimPath.claim("credit_score"),
            mandatory = true,
            display = mapOf(Locale.ENGLISH to "Credit Score"),
        )
    val ReportDate: ClaimDefinition =
        ClaimDefinition(
            path = ClaimPath.claim("report_date"),
            mandatory = true,
            display = mapOf(Locale.ENGLISH to "Report Date"),
        )
    val ValidUntil: ClaimDefinition =
        ClaimDefinition(
            path = ClaimPath.claim("valid_until"),
            mandatory = true,
            display = mapOf(Locale.ENGLISH to "Valid Until"),
        )
    val IssuingEntity: ClaimDefinition =
        ClaimDefinition(
            path = ClaimPath.claim("issuing_entity"),
            mandatory = true,
            display = mapOf(Locale.ENGLISH to "Issuing Entity"),
        )
